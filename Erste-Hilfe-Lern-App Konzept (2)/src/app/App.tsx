...
type AuthState = "lang_select" | "splash" | "login" | "register" | "paywall" | "app";
...
const UI: Record<string, Record<Lang, string>> = {
  ...
  version:            { DE:"ErstHilfe+ · Version 1.0.0", EN:"ErstHilfe+ · Version 1.0.0", TR:"ErstHilfe+ · Sürüm 1.0.0", AR:"ErstHilfe+ · الإصدار 1.0.0" },
...
function LangSelectScreen({onSelect}:{onSelect:(l:Lang)=>void}){
  return(
    <div className="min-h-screen bg-primary flex flex-col items-center justify-center px-8 text-white">
      <div className="w-20 h-20 bg-white rounded-2xl flex items-center justify-center mb-8 shadow-2xl"><Heart className="w-10 h-10 fill-primary text-primary"/></div>
      <h1 style={{fontFamily:"'Barlow Condensed',sans-serif"}} className="font-black text-5xl uppercase tracking-tight mb-2 text-center">ErstHilfe<span className="opacity-60">+</span></h1>
  ...
  return(
    <div className="flex flex-col min-h-screen bg-primary text-white relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 text-center relative z-10">
        <div className="w-20 h-20 bg-white rounded-2xl flex items-center justify-center mb-6 shadow-2xl"><Heart className="w-10 h-10 fill-primary text-primary"/></div>
        <h1 style={{fontFamily:"'Barlow Condensed',sans-serif"}} className="font-black text-5xl uppercase tracking-tight mb-2">ErstHilfe<span className="opacity-60">+</span></h1>
...
function PaywallScreen({lang,userName,onSuccess}:{lang:Lang;userName:string;onSuccess:()=>void}){
  ...
  return(
    <div className="min-h-screen bg-background flex flex-col">
      <div className="bg-primary px-5 pt-12 pb-6 text-white text-center">
        <div className="w-14 h-14 bg-white rounded-xl flex items-center justify-center mx-auto mb-3"><Heart className="w-7 h-7 fill-primary text-primary"/></div>
...
function TikTokFeed({lang,progress,onUpdate,onBack}:{lang:Lang;progress:ProgressData;onUpdate:(p:ProgressData)=>void;onBack:()=>void}){
  ...
  return(
    <div className="fixed inset-0 z-50 bg-black text-white flex flex-col select-none" style={{maxWidth:430,margin:"0 auto"}}>
      ...
      <div className="flex-1 flex flex-col pt-24 pb-4">
        {item.type==="video"&&(
          <div className="flex-1 relative">
            <img src={item.image} alt={item.title[lang]} className="absolute inset-0 w-full h-full object-cover"/>
            ...
            <div className="absolute bottom-6 left-5 right-5">
              <div className="inline-block bg-primary text-white text-xs font-bold px-2 py-1 rounded mb-2">VIDEO · {fmt(item.duration!)}</div>
              <h2 style={{fontFamily:"'Barlow Condensed',sans-serif"}} className="font-bold text-3xl uppercase">{item.title[lang]}</h2>
            </div>
          </div>
        )}
        {item.type==="infographic"&&(
          <div className="flex-1 relative">
            <img src={item.image} alt={item.title[lang]} className="absolute inset-0 w-full h-full object-cover opacity-50"/>
            ...
            <div className="absolute bottom-6 left-5 right-5">
              ...
              <h2 style={{fontFamily:"'Barlow Condensed',sans-serif"}} className="font-bold text-3xl uppercase mb-4">{item.title[lang]}</h2>
        ...
        {(item.type==="optional_quiz"||item.type==="mandatory_quiz")&&(
            ...
            {isMandatory&&(
              <div className="mb-4">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className={`font-mono font-bold text-sm ${timer<=5?"text-red-400":"text-white"}`}>{timer}s</span>
                </div>
                <div className="h-1.5 bg-white/20 rounded-full overflow-hidden">
                  <div className={`h-1.5 rounded-full transition-all duration-1000 ${timer<=5?"bg-red-500":"bg-primary"}`} style={{width:`${(timer/15)*100}%`}}/>
                </div>
              </div>
            )}
            {isOptional&&(
              <div className="mb-3 flex items-center justify-between">
                ...
                <button onClick={goNext} className="text-xs text-white/50 underline">{t("skip",lang)}</button>
              </div>
            )}
            <div className="bg-white/10 backdrop-blur rounded-2xl p-5 mb-5">
              <p style={{fontFamily:"'Barlow Condensed',sans-serif"}} className="font-bold text-2xl leading-tight" dir={lang==="AR"?"rtl":"ltr"}>{item.question?.[lang]}</p>
            </div>
            <div className="grid grid-cols-2 gap-3 flex-1">
              {item.options?.[lang].map((opt,i)=>{
                let cls="rounded-xl p-4 text-left text-sm font-medium border-2 transition-all ";
                if(!quizDone)cls+="border-white/20 bg-white/10 hover:bg-white/20 active:scale-95";
                else if(i===item.correct)cls+="border-green-400 bg-green-500/30";
                else if(i===quizSel&&i!==item.correct)cls+="border-red-400 bg-red-500/30";
                else cls+="border-white/10 bg-white/5 opacity-40";
                return(<button key={i} onClick={()=>handleAnswer(i)} className={cls} dir={lang==="AR"?"rtl":"ltr"}><span className="block text-xs opacity-60 mb-1 font-bold">{String.fromCharCode(65+i)}</span>{opt}</button>);
              })}
            </div>
            {quizDone&&(
              <div className={`mt-4 rounded-xl p-4 flex gap-3 ${quizSel===item.correct?"bg-green-500/20 border border-green-400/50":"bg-red-500/20 border border-red-400/50"}`}>
                {quizSel===item.correct?<Check className="w-5 h-5 text-green-400 flex-shrink-0 mt-0.5"/>:<X className="w-5 h-5 text-red-400 flex-shrink-0 mt-0.5"/>}
                <div>
                  {isMandatory&&quizSel===item.correct&&<p className="text-green-400 text-sm font-bold mb-1">+{Math.max(100,timer*20)} {t("pointsEarned",lang)}</p>}
                  <p className="text-sm text-white/80">{item.explanation?.[lang]}</p>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="flex items-center gap-3 px-5 pb-10 flex-shrink-0">
        {!isMandatory&&(<button onClick={idx>0?()=>setIdx(n=>n-1):onBack} className="w-12 h-12 rounded-full bg-white/10 flex items-center justify-center"><ChevronUp className="w-5 h-5"/></button>)}
        {(!isMandatory||quizDone)&&(
          <button onClick={goNext} className="flex-1 bg-primary text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition-transform" style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:18,letterSpacing:"0.05em",textTransform:"uppercase"}}>
            {idx===items.length-1?t("feedFinished",lang):t("next",lang)}<ChevronDown className="w-5 h-5"/>
          </button>
        )}
      </div>
    </div>
...
function EditProfileScreen({ lang, userName, onBack, onSave }: {
  ...
  const handleSave = () => {
    ...
    setSaved(true);
    onSave({ name, avatar, bio, location });
      ...
      <div className="flex-1 overflow-y-auto px-5 py-6 flex flex-col gap-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <p className="text-sm font-medium">{L("Emoji auswählen","Choose emoji","Emoji seç","اختر رمزاً تعبيرياً")}</p>
              <p className="text-xs text-muted-foreground">{L("Tippe auf ein Emoji unten","Tap an emoji below","Aşağıdan bir emoji seç","اضغط على رمز أدناه")}</p>
            </div>
          </div>
          <div className="grid grid-cols-8 gap-2">
            {AVATAR_EMOJIS.map(e => (
              <button key={e} onClick={() => setAvatar(e)}
                className={`text-2xl p-2 rounded-xl transition-all active:scale-90 ${avatar===e?"bg-primary/10 ring-2 ring-primary":"bg-card hover:bg-muted"}`}>
                {e}
              </button>
            ))}
          </div>
...
function ProfileMode({lang,userName,onLogout,school,onLicense,onLangChange,darkMode,setDarkMode,fontSize,setFontSize,autoplay,setAutoplay,onClearCache}:{
  ...
  const [displayAvatar, setDisplayAvatar] = useState(() => localStorage.getItem("eh_avatar") || "");
  ...
  if (showEditProfile) return <EditProfileScreen lang={lang} userName={userName} onBack={() => setShowEditProfile(false)} onSave={(d) => { setDisplayAvatar(d.avatar); setDisplayBio(d.bio); setDisplayLocation(d.location); setShowEditProfile(false); }}/>;

  return(
    <div className="pb-8">
      <div className="px-5 py-6 border-b border-border">
        <div className="flex items-start gap-4">
          <div className="w-16 h-16 bg-primary rounded-full flex items-center justify-center text-2xl flex-shrink-0">{displayAvatar || <span className="text-white font-bold">{userName.split(" ").map(w=>w[0]).join("").slice(0,2).toUpperCase()}</span>}</div>
          <div className="flex-1">
...
export default function App() {
  ...
  if(auth==="paywall")     return <PaywallScreen lang={lang} userName={userName} onSuccess={()=>setAuth("app")}/>;
